...
function __kool_perform_completion
    __kool_debug "Starting __kool_perform_completion"
    ...
    set -l requestComp "KOOL_ACTIVE_HELP=0 $args[1] __complete $args[2..-1] $lastArg"
...
# - Obtain the completions and store them in the global __kool_comp_results
# - Return false if file completion should be performed
function __kool_prepare_completions
    __kool_debug ""
    __kool_debug "========= starting completion logic =========="

    # Start fresh
    set --erase __kool_comp_results

    set -l results (__kool_perform_completion)
    ...
    if test $nospace -ne 0; or test $nofiles -eq 0
        ...
        __kool_debug "prefix: $prefix"

        set -l completions (string match -r -- "^$prefix.*" $__kool_comp_results)
        set --global __kool_comp_results $completions
        __kool_debug "Filtered completions are: $__kool_comp_results"

        # Important not to quote the variable for count to work
        set -l numComps (count $__kool_comp_results)
        ...
        if test $numComps -eq 1; and test $nospace -ne 0
            ...
            set -l split (string split --max 1 \t $__kool_comp_results[1])
            ...
            if not string match -r -q "[@=/:.,]" -- "$lastChar"
                ...
                set --global __kool_comp_results $split[1] $split[1].
                __kool_debug "Completions are now: $__kool_comp_results"
            end
        end
...
# The call to __kool_prepare_completions will setup __kool_comp_results
